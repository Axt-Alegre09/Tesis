/* JS/chatbot.brain.js
   Cerebro NLU liviano para catálogo + carrito + respuestas de producto.
*/
(() => {
  // =============== Utiles de lenguaje ===============
  ...
  const FLAVORS = [
    "carne","pollo","huevo","mandioca","queso","jamon","jamon y queso","saltena","salteña","combo",
    "dulce de leche","chocolate","maicena","vainilla","coco","membrillo","frutilla"
  ];
  // categorías (para “qué hay / sabores”)
  const CAT_SYNONYMS = {
  ...
  // =============== KB: precios y contenidos (lo que pasaste) ===============
  // clave: nombre canónico normalizado
  const KB = {
    // --- BOCADITOS ---
    "bocaditos combo 1": {
      precio: 55000,
      incluye: "2 empanadas (sabor a elegir), 2 sándwiches y 4 chipas"
    },
    "bocaditos combo 2": {
      precio: 50000,
      incluye: "3 empanadas (sabor a elegir), 3 sandwichitos, 2 pajagua, 2 chipaguazú, 4 chipas y 4 mbejú"
    },
    "bocaditos combo 3": {
      precio: 150000,
      incluye: "4 empanadas, 3 chipas, 3 chipaguazú, 3 sopas, 10 aperitivos, 5 pajagua, 4 milanesas y 5 mbejú"
    },
    "bocaditos combo 4": {
      precio: 75000,
      incluye: "9 sándwiches variados de milanesa (pollo y carne)"
    },
    "bocadito personal": {
      precio: 35000,
      incluye: "5 empanadas + 2 salsas (kétchup y lactonesa)"
    },
    "bocadito en pareja": {
      precio: 65000,
      incluye: "11 empanadas + 2 salsas (kétchup y lactonesa)"
    },

    // --- CONFITERÍA ---
    "alfajores": { precio: 25000, incluye: "Maicena y maicena bañada en chocolate" },
    "croissants": { precio: 30000 },
    "croisant": { precio: 30000 }, // alias por si aparece con una sola s
    ...
    "torta dulce de leche (pequeña congelada)": { precio: 45000 },
    "pai de manzana": { precio: 35000 },
    "pai de allana": { precio: 35000 }, // por si el texto aparece así
    ...
    "combo empanada + coca 250 ml": {
      precio: 24000,
      incluye: "1 empanada de carne, 1 pan y 1 Coca 250 ml"
    },
    "combo empanada saltena + salsa": {
      precio: 26000,
      incluye: "1 empanada salteña + 1 potecito de salsa (kétchup o lactonesa)"
    },
    "combo sandwich milanesa + papas + coca 350 ml": {
      precio: 25000,
      incluye: "1 sándwich de milanesa (carne), papas fritas pequeñas y 1 Coca 350 ml"
    }
  };
  ...
  const ALIAS = new Map([
    ...
    ["torta dulce de leche","torta dulce de leche (pequeña congelada)"],
    ["pai de manzana","pai de manzana"],["pai allana","pai de allana"],
    ...
    ["empanada huevo","empanada de huevo"],
    ["empanada mandioca","empanada de mandioca"],
    ["empanada jamon y queso","empanada de jamon y queso"],
  ...
  function getProductIndex(){
    ...
    for (const p of all) {
      ...
      // sabores que aparezcan en el nombre
      FLAVORS.forEach(f => { if (base.includes(normalize(f))) tokens.add(normalize(f)); });

      // indexar también alias que apunten a KB si matchean por nombre
      ...
      // también indexar el nombre entero
  ...
  function findProduct(text, flavor){
    ...
    if (!cands.length) {
      // si tenemos KB, intentar por alias → nombre de catálogo similar
      const kb = kbLookup(t);
      if (kb) {
        const all = getProductIndex().all;
  ...
  function parseMessage(msgRaw=""){
    ...
    // Info de producto (incluye / precio)
    ...
    if (/(precio|precios|cuanto sale|cuanto vale|valor)/.test(msg)) {
      const prodTxt = guessProductText(msg); // si no hay prod específico → category_prices
    ...
    // Setear cantidad: “pone 5 empanadas …”
  ...
  function extractFlavor(fragment){
    // tomar frases como "jamon y queso", "dulce de leche"
    ...
    // último fallback mono-palabra
    for (const f of FLAVORS) if (fragment.includes(f)) return normalize(f);
    return null;
    }

  // Extrae “2 empanadas de carne y 1 de jamon y queso”
  function extractItems(msg){
    const items = [];
    let lastProd = null;

    // 1) Bloques con cantidad + sustantivo + (opcional) "de <sabor...>"
    ...
    while ((m = r1.exec(msg)) !== null) {
      ...
      const prodTxt = (m[2] && normalize(m[2])) || guessProductText(msg) || lastProd || "empanadas";
      const flavor = m[3] ? normalize(m[3]) : extractFlavor(msg);
      lastProd = prodTxt;
      items.push({ cantidad: qty || 1, prodTxt, flavor });
    }

    // 2) Framentos “y 1 de jamon y queso”
  ...
  function guessProductText(msg){
    ...
    // alias/KB primero (mejor exactitud para combos)
    ...
    // si hay KB que contenga la frase
  ...
  async function actRemove(items){
    ...
    for (const it of (items?.length ? items : [{ cantidad:1, prodTxt: guessProductText("") }])) {
      ...
      if (isRemote) return { text:"En este momento puedo quitar por item en el carrito (remoto). Abrí el carrito y tocá la papelera 😉" };
  ...
  async function actSetQty({ prodTxt, flavor, qty }){
    ...
    if (snap.mode === "remote") return { text:"Por ahora solo puedo cambiar cantidades desde la vista del carrito (remoto). Usá los botones +/− 😊" };
  ...
  async function actEmpty(){
    ...
    if (snap.mode === "remote") return { text:"Para vaciar el carrito remoto, usá el botón “Vaciar carrito” en la página del carrito. 😉" };
...
• “sabores de empanadas”` };
  ...
  async function actProductInfo({ topic, prodTxt }){
    ...
    if (!kb) {
      // último intento: si coincide con algo del catálogo, devolvés precio del catálogo
    ...
    // si solo hay precio o solo incluye
  ...
  async function actCategoryPrices({ cat }){
    // listar precios que haya en KB para esa categoría
    const lines = [];
