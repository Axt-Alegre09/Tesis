// ==================== MÓDULO DE CONFIGURACIÓN - VERSIÓN PERFECTA ====================
// ✅ Usa instancia única de Supabase
// ✅ Tablas correctas: perfiles_usuarios, configuracion, business_info
// ✅ Sistema de notificaciones integrado
// ✅ Gestión completa de usuarios y roles

import { supa } from '../supabase-client.js';

// Variable para función de notificaciones
...
function setupTabs() {
  ...
  tabs.forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      if (targetTab === 'usuarios') cargarUsuarios();
      else if (targetTab === 'notificaciones') cargarNotificaciones();
      else if (targetTab === 'mantenimiento') cargarEstadoMantenimiento();
    });
  });
}
...
async function cambiarRolUsuario(userId, rolActual) {
  ...
  try {
    ...
    await crearNotificacionGlobal('sistema', 'Rol Actualizado', `Se cambió el rol de un usuario a ${nuevoRol}`);
...
function renderizarNotificaciones(notificaciones) {
  ...
  container.innerHTML = notificaciones.map(notif => {
    ...
    const iconMap = {
      pedido: 'cart-check', sistema: 'gear', chatbot: 'robot',
      catering: 'calendar-event', promocion: 'tag', stock: 'box-seam'
    };
...
async function cargarConfiguracionGeneral() {
  try {
    const { data, error } = await supa.from('business_info').select('*').single();
...
async function guardarConfiguracionGeneral() {
  ...
  try {
    const { error } = await supa.from('business_info').update(datos).eq('id', 1);
    
    if (error) throw error;
    
    await crearNotificacionGlobal('sistema', 'Configuración Actualizada', 'Se actualizó la información del negocio');
...
async function toggleMantenimiento() {
  try {
    ...
    await crearNotificacionGlobal('sistema', 'Modo Mantenimiento', `Modo mantenimiento ${nuevoEstado ? 'activado' : 'desactivado'}`);
